...
(function() {
  var app, config, cp, fs, io, path, pdf, phantomjs, runscript, sendfile, server;
  ...
  io.on('connection', function(socket) {
    ...
    socket.on('needfile', function(data) {
      var resp;
      resp = {
        name: data['filename'],
        type: data['type'],
    ...
    return socket.on('convertpdf', function(html) {
      var filename;
      console.log(html);
      console.log('convertpdf called');
      filename = __dirname + '/report.pdf';
      return pdf.create(html, config).toFile(filename, function(err, res) {
        console.log("filename: " + filename);
        if (err) {
          return console.log(err);
        }
        console.log(res);
        return socket.emit('convertpdffinished', filename);
      });
    });
  });
